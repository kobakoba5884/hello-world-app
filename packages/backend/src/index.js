...
const main = async () => {
  ...
  const uri =
    `mongodb://${MONGO_INITDB_ROOT_USERNAME}:${MONGO_INITDB_ROOT_PASSWORD}@${MONGO_HOST}:${MONGO_PORT}/?authSource=admin&readPreference=primary&ssl=false&directConnection=true`;
  ...
  app.get("/", async (_, res) => {
    ...
    const result = await db.collection(MONGO_INITDB_COLLECTION).find().toArray();
    res.send(result);
  });
